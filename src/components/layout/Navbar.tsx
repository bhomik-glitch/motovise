...
export function Navbar() {
    ...
    return (
        <nav className="fixed top-6 left-0 right-0 z-50 px-6">
            <div className="max-w-7xl mx-auto flex items-center h-16 relative">
                        ...
                        <div className="flex items-center bg-black/85 backdrop-blur-xl border border-white/10 rounded-full p-1.5 gap-1 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
                                    ...
                                    <Link
                                        ...
                                        className={cn(
                                            ...
                                            isActive
                                                ...
                                                : "text-white hover:text-white/80" // Products Link in White
                                        )}
                                    >
                                        {link.label}
                                    </Link>
                            ...
                            {actions.map((action) => (
                                <Link
                                    key={action.href}
                                    href={action.href}
                                    className="p-2 text-white hover:bg-white/10 rounded-full transition-all relative" // Icons in White
                                    title={action.label}
                                >
                ...
                {/* Mobile Trigger (on top of logo if needed, but here it's fine) */}
                <button
                    onClick={() => setMobileOpen((v) => !v)}
                    className="md:hidden z-30 p-2 text-white rounded-full bg-black/50 backdrop-blur-sm ml-auto"
                >
