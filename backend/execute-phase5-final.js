 ...
 * Final working version with correct auth handling
 */
...
// Get auth token - register new user or login existing
...
async function runTests() {
    ...
    try {
        ...
        const { token: token1, userId: userId1 } = await getAuthToken(user1Email);
        log(`User: ${user1Email} (${userId1})`);
        ...
        const { token: token2, userId: userId2 } = await getAuthToken(user2Email);
        ...
        const { token: token4, userId: userId4 } = await getAuthToken(user4Email);
        ...
        const { token: token5a } = await getAuthToken(user5aEmail);
        const { token: token5b } = await getAuthToken(user5bEmail);
